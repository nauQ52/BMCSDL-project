...
{
    public partial class fMultiEncryption : Form
    {
        OracleConnection conn;
        MultiEncryptionOracle multiEncryption;
        public fMultiEncryption()
        {
            InitializeComponent();
            CenterToParent();
            conn = Database.Get_connect();
            multiEncryption = new MultiEncryptionOracle(conn);
        }
        ...
        private void btnEncrypt_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataRow row in dataTable.Rows)
            {
                foreach (DataColumn column in dataTable.Columns)
                {
                    ...
                    {
                        string encryptedText = multiEncryption.EncryptMulti(plaintext, key);
        ...
        private void btnDecrypt_Click(object sender, EventArgs e)
        {
            ...
            // Duyệt qua từng dòng và cột để giải mã
            foreach (DataRow row in dataTable.Rows)
            {
                foreach (DataColumn column in dataTable.Columns)
                {
                    // Kiểm tra nếu dữ liệu trong cột là chuỗi và không rỗng
                    if (column.DataType == typeof(string))
                    {
                        string ciphertext = row[column]?.ToString();
                        if (!string.IsNullOrEmpty(ciphertext))
                        {
                            string decryptedText = multiEncryption.DecryptMulti(ciphertext, key);
